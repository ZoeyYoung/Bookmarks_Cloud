...
REGEXES = {
    ...
    'trimRe': re.compile('^\s+|\s+$/', re.I),
    'normalizeRe': re.compile('\s{2,}/', re.I),
    'killBreaksRe': re.compile('(<br\s*\/?>(\s|&nbsp;?)*){1,}/', re.I),
...
class Document:
    ...
    def description(self):
        return get_description(self._html(True))

    def keywords(self):
        return get_keywords(self._html(True))
    ...
    def select_best_candidate(self, candidates):
        sorted_candidates = sorted(
            list(candidates.values()), key=lambda x: x['content_score'], reverse=True)
    ...
    def transform_misused_divs_into_paragraphs(self):
        for elem in self.tags(self.html, 'div'):
            ...
            if not REGEXES['divToPElementsRe'].search(
                    # "".join(str(map(tostring, list(elem))))):
                    ''.join(str(map(tostring, list(elem))))):
...
def main():
    ...
    try:
        print((Document(file.read(), debug=options.verbose, url=options.url).summary().encode(enc, 'replace')))
    finally:
        file.close()
