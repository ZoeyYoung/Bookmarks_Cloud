...
from goose.cleaners import StandardDocumentCleaner, text_content, clean_tags
...
from goose.utils.encoding import get_charset, replace_meta_charset, force_meta
...
class Crawler(object):
    ...
    def get_document(self, url, raw_html):
        ...
        # output = open('get_html.html', 'w', encoding='utf-8')
        # print(raw_html, file=output)
        # import lxml.html
    ...
    def relase_resources(self, article):
        path = os.path.join(self.config.local_storage_path, '%s_*' %
                            article.link_hash)
