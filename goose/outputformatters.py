...
class OutputFormatter(object):
    ...
    def get_formatted_text(self, article):
        ...
        self.make_img_responsive()
        return self.parser.nodeToString(self.top_node)
    ...
    def make_img_responsive(self):
        for e in self.parser.getElementsByTag(self.top_node, tag='img'):
            self.parser.setAttribute(e, 'class', 'img-responsive')
    ...
    def replace_with_text(self):
        ...
        """
        self.parser.stripTags(self.get_top_node(), 'b',
                              'strong', 'i', 'br', 'sup')
    ...
    def remove_fewwords_paragraphs(self, article):
        ...
        for el in all_nodes:
            ...
            if 'br hr img object embed video'.find(tag) != -1:
                continue
            text = self.parser.getText(el)
            empty_text_pattern = re.compile("^[\s|&nbsp;]*$", re.I)
            # stop_words = self.stopwords_class(language=self.get_language(article)).get_stopword_count(text)
            if text != '\\r' and empty_text_pattern.search(text) is not None \
                    and len(self.parser.getChildren(el)) == 0:
